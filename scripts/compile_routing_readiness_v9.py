...
def build_stp_lookup(stp_df: pd.DataFrame) -> tuple[dict[str, dict[str, float]], dict[int, dict[str, float]]]:
    by_tag: dict[str, dict[str, float]] = {}
    by_step: dict[int, dict[str, float]] = {}
    if stp_df.empty:
        return by_tag, by_step
    numeric_cols = [col for col in stp_df.columns if pd.api.types.is_numeric_dtype(stp_df[col])]
    if "tag" in stp_df.columns:
        for tag, group in stp_df.groupby("tag", sort=False):
            row = group.iloc[0]
            by_tag[str(tag)] = {col: float(row[col]) for col in numeric_cols if pd.notna(row[col])}
    if "step" in stp_df.columns:
        for step, group in stp_df.groupby("step", sort=False):
            row = group.iloc[0]
            by_step[int(step)] = {col: float(row[col]) for col in numeric_cols if pd.notna(row[col])}
    return by_tag, by_step


def attach_stp_row(tag: str, step: int, by_tag: dict[str, dict[str, float]], by_step: dict[int, dict[str, float]]) -> dict[str, float]:
    if tag in by_tag:
        return dict(by_tag[tag])
    if step in by_step:
        return dict(by_step[step])
    return {}


def transition_balance(value: float) -> float:
    if not math.isfinite(value) or value <= 0.0:
        return float("nan")
    return float(-abs(math.log(value)))
...
def main() -> None:
    ...
    parser.add_argument("--stp-metrics-csv", type=str, default="")
    parser.add_argument("--output-tag", type=str, required=True)
    parser.add_argument("--output-dir", type=str, default="results/readiness_v5_stp")
    ...
    stp_df = pd.DataFrame()
    if args.stp_metrics_csv:
        stp_path = Path(args.stp_metrics_csv)
        if stp_path.exists():
            stp_df = pd.read_csv(stp_path)
    stp_by_tag, stp_by_step = build_stp_lookup(stp_df)
    ...
    for tag in args.tags:
        ...
        row = {
            ...
            "step": step,
            "val_loss": (
                float(metric_rows.iloc[-1]["val_loss"])
                if not metric_rows.empty and pd.notna(metric_rows.iloc[-1]["val_loss"])
                else np.nan
            ),
            ...
            "bank_tail_frac_050": float(
                bank_subset[bank_subset["metric"] == "fraction_oracle_headroom_gt_0.050"]["mean"].iloc[0]
            ),
        ...
        row.update(attach_stp_row(tag=tag, step=step, by_tag=stp_by_tag, by_step=stp_by_step))
        rows.append(row)

    df = pd.DataFrame(rows).sort_values("step").reset_index(drop=True)
    if df.empty:
        raise ValueError("No readiness-v9 rows found.")

    if "transition_ratio_mean" in df.columns:
        df["transition_balance_mean"] = df["transition_ratio_mean"].map(transition_balance)
    if "transition_ratio_final" in df.columns:
        df["transition_balance_final"] = df["transition_ratio_final"].map(transition_balance)
    ...
    optional_specs = {
        "tube_loss_mean_z": ("tube_loss_mean", False),
        "tube_loss_final_z": ("tube_loss_final", False),
        "tube_stability_gap_z": ("tube_stability_gap", False),
        "norm_b0_final_mean_z": ("norm_b0_final_mean", True),
        "transition_balance_mean_z": ("transition_balance_mean", True),
        "transition_balance_final_z": ("transition_balance_final", True),
    }
    for z_col, (source_col, larger_is_better) in optional_specs.items():
        if source_col in df.columns:
            components[z_col] = zscore(df[source_col].to_numpy(), larger_is_better=larger_is_better)
    ...
    weights = {
        ...
        "tube_loss_mean_z": 0.25,
        "tube_loss_final_z": 0.40,
        "tube_stability_gap_z": 0.20,
        "norm_b0_final_mean_z": 0.20,
        "transition_balance_mean_z": 0.20,
        "transition_balance_final_z": 0.20,
    }
    ...
    df["routing_readiness_v5_stp"] = scores

    output_path = out_dir / f"{args.output_tag}_routing_readiness_v5_stp.csv"
