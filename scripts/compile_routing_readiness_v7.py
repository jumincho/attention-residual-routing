...
def main() -> None:
    ...
    parser.add_argument("--selector-dir", type=str, default="results/regret_reduction_v7")
    ...
    out_dir = ROOT / "results" / "readiness_v7"
    ...
    for tag in args.tags:
        ...
        if args.selector_prefix:
            ...
            row["selector_dev_regret"] = find_selector_dev_metric(
                selector_dir=selector_dir,
                selector_prefix=args.selector_prefix,
                step=step,
                feature_mode=args.feature_mode,
            ...
            row["selector_dev_top1"] = find_selector_dev_metric(
                selector_dir=selector_dir,
                selector_prefix=args.selector_prefix,
                step=step,
                feature_mode=args.feature_mode,
    ...
    if "selector_dev_delta" in df.columns:
        ...
        components["selector_dev_regret_z"] = zscore(df["selector_dev_regret"].to_numpy(), larger_is_better=False)
        components["selector_dev_top1_z"] = zscore(df["selector_dev_top1"].to_numpy(), larger_is_better=True)
    ...
    weight_map = {
        ...
        "selector_dev_delta_z": 1.10,
        "selector_dev_fraction_z": 0.40,
        "selector_dev_regret_z": 1.10,
    ...
    df["routing_readiness_v3"] = scores
    df.to_csv(out_dir / f"{args.output_tag}_routing_readiness_v3.csv", index=False)
