...
RESULTS_DIR = ROOT / "results" / "regret_reduction_v9"
FREEZE_DIR = ROOT / "results" / "lockbox_manifests_v9"
DEFAULT_MODELS = [
    "rf_pair",
    "rf_pair_weighted",
    "hgb_pair",
...
def selection_path(seed: int, step: int, bank_size: int, feature_mode: str) -> Path:
    return RESULTS_DIR / f"v9_ccnews_seed{seed}_full_step{step}_dev2048_b{bank_size}_{feature_mode}_model_selection.csv"


def summary_path(seed: int, step: int, bank_size: int, feature_mode: str) -> Path:
    return RESULTS_DIR / f"v9_ccnews_seed{seed}_full_step{step}_dev2048_b{bank_size}_{feature_mode}_summary.csv"
...
def write_freeze_ledger(winners: pd.DataFrame, freeze_path: Path) -> None:
        ...
        rows.append(
            {
                ...
                "selection_csv": str(row.get("selection_csv", "")),
                "summary_csv": str(row.get("summary_csv", "")),
                "freeze_scope": "dev_select_v9",
                "final_splits": "final_D final_E final_F",
                "final_opened": 0,
            }
        )
    pd.DataFrame(rows).to_csv(freeze_path, index=False)
...
def main() -> None:
    ...
    parser.add_argument(
        "--freeze-ledger",
        type=str,
        default=str(FREEZE_DIR / "v9_ccnews_p256d64_lockbox_selection_freeze.csv"),
    )
    ...
    if winners:
        ...
        write_freeze_ledger(winners_df, Path(args.freeze_ledger))
