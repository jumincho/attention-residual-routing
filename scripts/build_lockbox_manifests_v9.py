...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=str, default=str(ROOT / "results" / "lockbox_manifests_v9"))
    parser.add_argument("--plots-dir", type=str, default=str(ROOT / "results" / "plots"))
...
def main() -> None:
    ...
    for split_name, target in partitions:
        ...
        print(
            f"[lockbox-manifest-v9] split={split_name} created={len(partition_rows[split_name])} "
    ...
    plt.tight_layout()
    plt.savefig(plot_dir / f"lockbox_split_checks_v9_{args.tag}_window_counts.png", dpi=160)
    plt.close()
    ...
    plt.xticks(rotation=20)
    plt.tight_layout()
    plt.savefig(plot_dir / f"lockbox_split_checks_v9_{args.tag}_document_counts.png", dpi=160)
    plt.close()
    ...
    if not overlap_df.empty:
        ...
        plt.tight_layout()
        plt.savefig(plot_dir / f"lockbox_split_checks_v9_{args.tag}_overlap.png", dpi=160)
        plt.close()
