...
CORPUS_TAG="${CORPUS_TAG:?set CORPUS_TAG=v8_ccnews_p256d64_lockbox}"
FINAL_SPLIT="${FINAL_SPLIT:-final_A}"
...
FINAL_MANIFEST="${FINAL_MANIFEST:?set FINAL_MANIFEST=/abs/path/to/final.jsonl}"
...
SELECTOR_MODEL="${SELECTOR_MODEL:-rf_pair}"
OUTPUT_PREFIX="${OUTPUT_PREFIX:?set OUTPUT_PREFIX=v8_locked_ccnews_step3000_b32}"
...
FINAL_TAG="${FINAL_TAG:-${CORPUS_TAG}_locked_${FINAL_SPLIT}}"
...
run_oracle_group() {
  ...
  if [[ -f "results/oracles/${prefix}_oracle_mask_alignment_summary.csv" ]]; then
    echo "[lockbox-v8] skip oracle ${prefix}"
    return
  fi
